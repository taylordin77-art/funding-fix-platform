/*
# Persist Assessment Action Plan RPC — array-append fix

## Purpose
Fixes a runtime error in persist_assessment_action_plan: the inserted-id
collection used `v_inserted_ids[array_length(...)+1] := ...` which fails with
"cannot subscript type uuid because it does not support subscripting" on
out-of-bounds append. Replaced with array_append.

## Changes
- DROP + CREATE OR REPLACE of persist_assessment_action_plan (same signature,
  SECURITY DEFINER, SET search_path = public). All validation logic identical
  to the previous version; only the RETURNING id capture changed.
- Re-grants EXECUTE to authenticated, revokes from PUBLIC.
- No schema, table, column, trigger, or RLS changes.
*/
...
BEGIN
  ...
  weak_refs AS (
    SELECT DISTINCT aa.pillar || ':' || aa.question_index AS ref
